...
class AbsenController extends Controller
{
    ...
    public function absen()
    {
        ...
        if(Auth::user()->isOrtu()){
            $ortu = Auth::user()->ortu->id;
            $siswa = Siswa::where('ortu_id', $ortu)->get()->pluck('kelas_id')->toArray();
            $kelas_siswa = array_unique($siswa);
            $jadwals = JadwalBelajar::whereIn('kelas_id', $kelas_siswa)
            ->whereHas("kelas.siswa")
            ->orderBy(Kelas::select('kode')->whereColumn('id', 'kelas_id'), 'asc')
            ->get();
        }
        ...
        if(Auth::user()->isGuru())
        {
            ...
            return view('dashboard.ortu.laporan.absen.index', [
                'jadwals' => $jadwals
            ]);
        }
    }
    ...
    public function fetchDataAbsen(Request $request)
    {
        if (request()->ajax()) {
            ...
            if(Auth::user()->isGuru())
            {
                ...
                $output = view('dashboard.ortu.laporan.absen._data-nilai', [
                    'jadwal' => $jadwal,
                    'kelas_id' => $kelas_id,
                    'mapel_id' => $mapel_id,
                ]);
            ...
            return response()->json($output->render());
        } else {
            abort(404);
        }
    }
    ...
    public function tableDataAbsen(Request $request)
    {
        if (request()->ajax()) {
            ...
            if(Auth::user()->isGuru())
            {
                $jadwals = $jadwals->where('guru_id', Auth::user()->guru->id);
            }
            ...
            if(Auth::user()->isOrtu())
            {
                // $data = $data->where('ortu_id', Auth::user()->ortu->id);
            }
    ...
    public function exports($kelas_id, $mapel_id)
    {
        ...
        if(Auth::user()->isOrtu()) {
            $siswa = Siswa::with(['absens', 'kelas', 'programkeahlian'])
                ->where('kelas_id', $kelas->id)
                ->where('ortu_id', Auth::user()->ortu->id)
                ->orderBy('nama', 'asc')
                ->get(); 
        }
