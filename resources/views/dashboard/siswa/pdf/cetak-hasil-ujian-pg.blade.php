...
@section('content')
    ...
    <table align="center">
        ...
        <tr class="satu">
            <td>Mulai Ujian</td>
            <td>: {{ Carbon\Carbon::parse($siswa->started_at)->format('H:i:s') }} -
                {{ Carbon\Carbon::parse($siswa->ended_at)->format('H:i:s') }} WIB</td>
        </tr>
        ...
        <tr>
            <td>Jumlah Salah</td>
            <td>: {{ $siswa->ujianHasil->where('status', '0')->where('jawaban', '!=', null)->count() }}</td>
        </tr>
        ...
        <tbody>
            @foreach ($ujianHasil as $ujian)
                @php
                    ...
                    $jawabanSw == null ? ($jawabanSw = '-') : ($jawabanSw = $jawabanSw . '. ' . $soal['pilihan_' . $jawabanSw]);
                @endphp
